...
def displayDictGame(
    ...
    plat: list = [[" " for j in range(plateau["L"])]
                  for i in range(plateau["H"])]
...
def actionVaisseau(entree: str, vaiseau: dict) -> None or bool:
    if entree == "k":
        if(vaiseau["posx"] > 0):
            vaiseau["posx"] -= 1
    elif entree == "m":
        if(vaiseau["posx"] < plateau["L"] - 1):
            vaiseau["posx"] += 1
    elif entree == "o":
        return True
    else:
...
def fin_jeu(vaisseau: dict, liste_Aliens: list, plateau: dict) -> bool:
    return (len(liste_Aliens) == 0 or len(
        [x for x in liste_Aliens if x["posy"] == plateau["H"] - 1]) >= 1)
