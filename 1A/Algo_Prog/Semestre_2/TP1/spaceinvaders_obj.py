...
class Plateau:
    ...
    def afficherBandeau(self, jeu):
        ...
        print(f"  {jeu.score:5}  {jeu.vie : 5}     {jeu.niveau : 5}")
        Plateau.afficherLigne()
        ...
        for ligne in range(Plateau.hauteur):
            for col in range(Plateau.largeur):
                if (
                    posTir != None
                    and col == posTir[0]
                    and ligne < Plateau.hauteur - 1
                    and ligne > posTir[1]
                ):
                    print(
                        Plateau.typeTirs[jeu.vaisseau.nivTir - 1], end=Plateau.normalC
                    )
                    continue
                ...
                char: str = " "
                posA: List[Alien] = [
                    a for a in jeu.aliens if a.position() == (col, ligne)
                ]
                posP: List[any] = [
                    a for a in jeu.PowersUp if a.position() == (col, ligne)
                ]
        ...
        if aliens != None or len(aliens) > 0:
            for i in range(nombre):
                if len(aliens) > 0:
                    ...
                    if mort.mort():
                        PartieJeu.PowersUp.append(
                            PowerUp(mort.posX, mort.posY, mort.nivTir)
                        )
        ...
        while self.vie > 0 and action != "q":
            ...
            if self.finJeu(self.alienPlusBas()):
                ...
                for a in self.aliens:
                    if a.larguerBombe():
                        PartieJeu.PowersUp.append(
                            PowerUp(a.posX, a.posY, type_="bombe")
                        )
                for p in PartieJeu.PowersUp:
                    p.update(self)
...
class Menu:
    ...
    def affichageCentre(texte: str):
        print(
            " " * (Plateau.largeur // 2 - len(texte) // 2)
            + texte
            + " " * (Plateau.largeur // 2 - len(texte) // 2 - 1)
        )
